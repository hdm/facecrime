...
func (d *Detector) UnpackCascades() error {
	...
	faceFindercascade, err = d.ParseCascade("facefinder")
	if err != nil {
		return errors.New("error reading the facefinder cascade file")
	}
	// Unpack the binary file. This will return the number of cascade trees,
	// the tree depth, the threshold and the prediction from tree's leaf nodes.
	faceClassifier, err = p.Unpack(faceFindercascade)
	if err != nil {
	...
	puplocCascade, err = d.ParseCascade("puploc")
	if err != nil {
	...
	flpcs, err = d.parseFlpCascades("lps/")
	if err != nil {
...
func (d *Detector) parseFlpCascades(path string) (map[string][]*FlpCascade, error) {
	...
	for _, casc := range cascades {
		puplocCascade, err = d.ParseCascade(path + casc)
		if err != nil {
			d.Log("Error reading the cascade file: %v", err)
		}
		flpc, err := pl.UnpackCascade(puplocCascade)
		flpcs[casc] = append(flpcs[casc], &FlpCascade{flpc, err})
	}
	return flpcs, err
}
