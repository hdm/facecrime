...
import (
	...
	"github.com/hdm/facecrime/pigo/wasm/detector"
)
...
type Canvas struct {
	...
	faceCallback func(int, int, []int, *pigo.Puploc, *pigo.Puploc, [][]int)
}
...
func (c *Canvas) Render() error {
	...
	//
	// TODO: Find another way to explicitly stop/close this
	//
...
// processFaces calls back to the main ebitengine
func (c *Canvas) processFaces(dets [][]int) {
	faceCount := len(dets)
	if faceCount == 0 {
		c.faceCallback(faceCount, 0, nil, nil, nil, nil)
		return
	}
	for i := 0; i < len(dets); i++ {
		...
		landmarks := det.DetectLandmarkPoints(leftPupil, rightPupil)
		c.faceCallback(faceCount, i, dets[i], leftPupil, rightPupil, landmarks)
	}
}
